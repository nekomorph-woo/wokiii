...
2. `_review.md` 最新 Round 状态为 Converged，或 Max rounds 且无 🔴🟠 Open（或无 review 产出）
...
主 agent 不执行实现，spawn subagent 完成 TDD 循环，保持主 agent 上下文清洁。

1. 读 `_plan.md` → 找第一个 `[ ]` step，记录 step 编号 N
2. 启动 subagent：

   ```
   Agent({
     description: "implement step <N>",
     prompt: "调用 Skill('wok-implement')，参数 '--autopilot --step <N>'。读取 .wok-plans/<system-name>/_plan.md 定位 step <N>，执行完整 TDD 循环（RED→GREEN→REFACTOR）。完成后报告：step 完成状态、测试数量、是否阻塞。"
   })
   ```

3. 等待 subagent 完成
4. 读 `_plan.md` 确认 step N `[ ] → [x]`
5. step 标记 `⚠️ 阻塞` → 🛑 中断
...
检查 code-review 的 auto_fix 模式：
- `auto_fix = true`（system-name 非 `cr-` 前缀）→ **跳过此步骤**。code-review Stage 5 已分析 🟡，幂等保证无需重复调用
- `auto_fix = false` → 调用 Skill 工具加载 wok-cr-insight，参数 `--types yellow`

autopilot 场景下 system-name 非 `cr-` 前缀，auto_fix 始终为 true，步骤 C 始终跳过。
...
- cr-insight: <N> 🟡 analyzed (by code-review Stage 5) / skipped
```
