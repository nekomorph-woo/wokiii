...
```
/wok-code-review [--scope <范围>] [--focus <维度>] [--max-rounds <N>] [--fix]
```
...
| `--fix`        | flag                               | auto   | 强制启用修复（独立/cr 管道下使用） |
...
6. **检测修复模式**：
   - 检查 system-name 前缀：非 `cr-`（如 `feat-s-`、`fix-`、`exp-`）→ 管道内调用 → `auto_fix = true`
   - 前缀为 `cr-` 或无匹配目录 → 独立调用 → `auto_fix = false`
   - `--fix` 参数覆盖：显式传入时强制 `auto_fix = true`

**产出**：上下文包（含 files 列表、language、design\_anchors、prd\_summary、phase\_dir、auto\_fix）
...
| pr-test-analyzer      | 构建表格格式（scope\_files + test\_dir + prd\_context）           |
...
**`auto_fix = false` 时跳过此阶段。**

循环执行以下步骤，直到收敛或达到 `max-rounds`：

1. **洞察分析 🔴🟠**：

   * 执行 wok-cr-insight 的分析流程，参数为 `--types red,orange`

   * 每个 🔴🟠 获得结构化洞察（根因 + 修改方案 + 一致性评估）

   * wok-cr-insight 分析失败 → 跳过该 finding 的洞察，降级为直接修复
2. **基于洞察修复 🔴🟠**：
...
触发 `wok-cr-insight` 分析问题，追加根因分析和修改方案到 `_review.md`。

调用方式：直接执行 cr-insight 的分析流程。按 `auto_fix` 决定分析范围：

| `auto_fix` | `--types` 参数 | 原因 |
|---|---|---|
| `true` | `yellow` | 🔴🟠 已在 Stage 3 获得洞察并修复，此步骤补全 🟡 分析 |
| `false` | `all` | Stage 3 已跳过，🔴🟠🟡 全部需要洞察分析供用户决策 |
...
* `auto_fix = false` 时同步跳过 Stage 3 和 Stage 3 的 simplify
