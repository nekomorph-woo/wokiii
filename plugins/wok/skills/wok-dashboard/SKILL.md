...
- **Windows**（`$env:OS` 为 `Windows_NT`，或 shell 为 PowerShell）→ 运行 `deploy.ps1 <system-name>`
- **macOS / Linux** → 运行 `deploy.sh <system-name>`
...
- **必须通过部署脚本管理 server** — DO NOT 手动运行 `python3 _server.py` 或自行选择端口。Server 的启动、复用、重启全部由 `deploy.sh`（Unix）或 `deploy.ps1`（Windows）处理
